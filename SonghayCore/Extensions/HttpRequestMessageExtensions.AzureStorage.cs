...
namespace Songhay.Extensions
{
    public static partial class HttpRequestMessageExtensions
    {
        ...
        /// </remarks>
        public static AuthenticationHeaderValue ToAzureStorageAuthorizationHeader(this HttpRequestMessage request, DateTime requestMoment, string storageAccountName, string storageAccountKey, string eTag, string md5)
        {
            if (request == null) throw new ArgumentNullException(nameof(request));

            byte[] signatureBytes = request.ToAzureStorageSignature(storageAccountName, eTag, md5);
        ...
        public static byte[] ToAzureStorageSignature(this HttpRequestMessage request, string storageAccountName, string eTag, string md5)
        {
            ...
            if (method == HttpMethod.Put)
            {
                ...
                {
                    throw new NullReferenceException($"The expected Content Headers are not here. Was {nameof(HttpRequestMessage)}.{nameof(HttpRequestMessage.Content)} set?", ex);
                }
            }
        ...
        public static HttpRequestMessage WithAzureStorageHeaders(this HttpRequestMessage request, DateTime requestMoment, string serviceVersion, string storageAccountName, string storageAccountKey, string eTag, string md5)
        {
            ...
            request.Headers.Authorization = request.ToAzureStorageAuthorizationHeader(requestMoment, storageAccountName, storageAccountKey, eTag, md5);
