...
public static partial class IActivityExtensions
{
    ...
    public static ActivityOutput<TOutput> StartActivityForOutput<TInput, TOutput>(this IActivity activity, TInput input, TraceSource traceSource)
    {
        return activity.StartActivityForOutput<TInput, TOutput>(input, traceSource, traceWriterGetter: null, flushLog: true);
    }
    ...
    /// <param name="flushLog">when <c>true</c> return <see cref="TraceSource"/> log</param>
    /// <returns>The the <see cref="TraceSource"/> log when <c>flushLog</c> is <c>true</c>.</returns>
    public static ActivityOutput<TOutput> StartActivityForOutput<TInput, TOutput>(this IActivity activity, TInput input, TraceSource traceSource, Func<TextWriter> traceWriterGetter, bool flushLog)
    {
        using (var writer = traceWriterGetter?.Invoke() ?? new StringWriter())
        using (var listener = new TextWriterTraceListener(writer))
        {
