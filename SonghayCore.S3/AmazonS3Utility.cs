...
public static class AmazonS3Utility
{
    ...
    public static async Task<EndpointContentResult<IReadOnlyCollection<StorageObject>>> CollectS3ObjectsFromPaginationAsync(AmazonS3Client? s3Client, ListObjectsV2Request? request, ILogger logger)
    {
        ...
        ILogger logger)
    {
        ...
        if (s3Client != null)
        {
            logger.LogDebug("{Name} found. Returning...", nameof(AmazonS3Client));

            return s3Client;
        }
        else
        {
            logger.LogDebug("{Name} not found. Attempting without credentials chain...", nameof(AmazonS3Client));
        }
        ...
        ILogger logger)
    {
        ...
        {
            logger.LogError("The expected {Name} is not here. AWS environment variables not set! Returning...",
